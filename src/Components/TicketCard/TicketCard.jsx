    ...
    <div onClick={() => handleAddTask(ticket)} className='card shadow-sm bg-white cursor-pointer transition-all duration-300 hover:scale-105 hover:shadow-xl'>
      <div className='p-5 space-y-3'>
        <div className='flex justify-between items-center'>
          <h3 className='card-title w-[290px]'>{title}</h3>
          <p className={`${status === "Open" ? "bg-[#B9F8CF]" : "bg-[#F8F3B9]"} rounded-full px-3 md:px-4  text-[10px] md:text-sm py-2 font-semibold flex items-center`}><span className={`badge badge-xs mr-2 ${status === "Open" ?  " badge-success " : "badge-warning"}`}></span> {status}</p>
        </div>
        ...
        <div className='flex flex-col lg:flex-row lg:justify-between items-center'>
          <div className='flex items-center w-full lg:w-fit justify-between lg:justify-start space-x-4  text-gray-600'>
            <p className=''>#{id}</p>
            <p className={` ${priority === "High" ? "text-red-500" : priority === "Medium" ? "text-warning" : "text-green-600"} `}>{priority} Priority</p>
          </div>
          <div className='flex items-center w-full lg:w-fit justify-between lg:justify-start space-x-4 text-gray-600'>
